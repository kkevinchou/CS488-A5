...
-- metal_ref_mat = gr.texture_material("../data/metal2.png", {0.5, 0.4, 0.8}, 100, 0.06, 0.35)
-- metal_mat = gr.texture_material("../data/metal2.png", {0.5, 0.4, 0.8}, 100)
metal_door_mat = gr.texture_material("../data/metal_door_texture3.png", {0.5, 0.4, 0.8}, 100)
life_saver_mat = gr.texture_material("../data/stripe.png", {0.5, 0.4, 0.8}, 100)

off_white_reflective = gr.material({0.65, 0.6, 0.65}, {0.5, 0.4, 0.8}, 100, 0.06, 0.35)
off_white = gr.material({0.65, 0.6, 0.65}, {0.5, 0.4, 0.8}, 100)
...
crate1:scale(0.3, 0.34, 0.29)
crate1:set_material(crate_mat)
plane:add_child(crate1)

life_saver = gr.torus('life_saver')
life_saver:translate(-1.01, 0.2, 0.1)
life_saver:scale(0.1, 0.1, 0.1)
life_saver:set_material(life_saver_mat)
plane:add_child(life_saver)

metal_door = gr.cube('metal_door')
metal_door:translate(-1.5, -0.5, 0.1)
metal_door:scale(0.3, 0.7, 0.3)
metal_door:set_material(metal_door_mat)
plane:add_child(metal_door)
...
t1 = gr.tween('rotate', 0, 0.5, {0.4, 0, -1})
t2 = gr.tween('rotate', 1.5, 2, {0, 0, -1})
t3 = gr.tween('rotate', 2, 2.5, {-0.4, 0, -1})
t4 = gr.tween('rotate', 3.5, 4, {0, 0, -1})
t5 = gr.tween('translate', 4, 5.5, {0, 0, 1})

-- gr.render(scene_root, 'scene.png', 500, 500,
--       {0, 0, 2.3}, {0, 0, -1}, {0, 1, 0}, 50,
--       {0.25, 0.25, 0.25}, {box_light}, {t1, t2, t3, t4, t5})

gr.render(scene_root, 'scene.png', 500, 500,
      {0, 0, 2.3}, {0, 0, -1}, {0, 1, 0}, 50,
      {0.25, 0.25, 0.25}, {window_light}, {t1, t2, t3, t4, t5}, 5.5, 30)